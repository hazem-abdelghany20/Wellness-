-- Wellness+ Demo Seed (supplementary, prod-safe)
-- Idempotent — safe to run against any environment.
-- Adds realistic rows for the HR/Admin dashboard sections that were
-- previously rendering from hardcoded HR_DATA / ADMIN_DATA fallbacks.
--
-- Designed to run on the deployed Supabase project without depending on
-- local seed users — created_by / actor_id are left NULL when the demo
-- employee profiles aren't present. Local devs running `supabase db reset`
-- will get richer joins automatically because the demo profiles do exist.

-- Resolves the seeded Wellhouse Group company id (deterministic UUID).
-- If you renamed the company in your env, swap the constant below.

-- ── Check-in history (only if demo employees exist locally) ──
...
FROM (
  SELECT id, company_id,
    CASE display_name
      WHEN 'Amira Hassan' THEN 0 WHEN 'Yusuf Naguib' THEN 1
      WHEN 'Lina Farouk'  THEN 2 WHEN 'Omar Sami'    THEN 3
      WHEN 'Nadia Kamel'  THEN 4 WHEN 'Sara Anwar'   THEN 5
    END AS bias
...
-- ── Challenge participants for the Sleep Sprint (when employees present)
INSERT INTO public.challenge_participants (challenge_id, user_id, company_id, score)
SELECT
  '00000000-0000-0000-0003-000000000001',
  p.id, p.company_id, (40 + random() * 60)::numeric(5,1)
...
WHERE p.company_id = '00000000-0000-0000-0000-000000000001'
  ...
  AND EXISTS (SELECT 1 FROM public.challenges WHERE id = '00000000-0000-0000-0003-000000000001')
ON CONFLICT (challenge_id, user_id) DO NOTHING;

-- ── Broadcasts (created_by NULL — FK-safe everywhere) ────────
INSERT INTO public.broadcasts (id, company_id, title_en, title_ar, body_en, body_ar, scope, status, scheduled_at, sent_at, created_by) VALUES
  ('00000000-0000-0000-3000-000000000001', '00000000-0000-0000-0000-000000000001',
   ...
   'all', 'sent',  now() - INTERVAL '30 days', now() - INTERVAL '30 days', NULL),
  ('00000000-0000-0000-3000-000000000002', '00000000-0000-0000-0000-000000000001',
   ...
   'all', 'sent',  now() - INTERVAL '14 days', now() - INTERVAL '14 days', NULL),
  ('00000000-0000-0000-3000-000000000003', '00000000-0000-0000-0000-000000000001',
   ...
   'all', 'sent',  now() - INTERVAL '5 days', now() - INTERVAL '5 days', NULL),
  ('00000000-0000-0000-3000-000000000004', '00000000-0000-0000-0000-000000000001',
   ...
   'all', 'scheduled', now() + INTERVAL '4 days', NULL, NULL),
  ('00000000-0000-0000-3000-000000000005', '00000000-0000-0000-0000-000000000001',
   ...
   'all', 'cancelled', now() - INTERVAL '2 days', NULL, NULL)
ON CONFLICT (id) DO NOTHING;

-- ── Integrations (3 healthy + 1 pending) ─────────────────────
...
-- ── Audit log (actor_id NULL — actor_email kept for display) ─
INSERT INTO public.audit_log (id, occurred_at, actor_id, actor_email, action, target, target_id, severity, details) VALUES
  ('00000000-0000-0000-6000-000000000001', now() - INTERVAL '6 hours',
   NULL, 'sara.hr@demo.wellhouse.test',
