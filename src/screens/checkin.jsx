...
import { typeStyles, Icon, Button, Ring, Slider } from '../design-system.jsx';
import { TopBack } from './onboarding.jsx';

export function ScreenCheckIn({ theme, t, dir, go, variant = 'sliders', state }) {
  const T = theme;
  ...
  if (saved) {
    return (
      <div style={{ height: '100%', background: T.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 28, textAlign: 'center' }}>
        ...
        <div style={{ fontFamily: typeStyles(T).displayFont, fontSize: 30, color: T.text, marginTop: 26, letterSpacing: -0.5 }}>
          {t('checkInDone')}
        </div>
...
function VarEmoji({ theme, q, v, set }) {
  const T = theme;
