...
import {
  typeStyles, Icon, AvatarDisplay, AVATAR_OPTIONS,
  Button, Card, Ring, Slider,
...
export function TopBack({ theme, onBack, dir }) {
  return (
...
// ── Join ──────────────────────────────────────────────────────
export function ScreenJoin({ theme, t, onNext, dir }) {
  const T = theme;
  const [code, setCode] = useState('WH-4782');
  return (
    <ScreenFrame theme={T}>
      <div style={{ padding: '30px 22px 14px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        <img src="wellness-mark.png" alt="Wellness+" style={{ height: 26, width: 26 * (551 / 274) }}/>
        <div style={{ flex: 1 }} />
        <div style={{
        ...
        <Button theme={T} onClick={onNext} iconR="arrow" style={{ width: '100%' }}>{t('continue')}</Button>
...
// ── OTP ───────────────────────────────────────────────────────
export function ScreenOTP({ theme, t, onNext, onBack, dir }) {
  const T = theme;
  const [digits, setDigits] = useState(['4', '7', '2', '0', '', '']);
  const refs = useRef([]);
...
// ── Consent ───────────────────────────────────────────────────
export function ScreenConsent({ theme, t, onNext, onBack, dir }) {
  const T = theme;
  const items = [
  ...
  return (
    <ScreenFrame theme={T}>
      <div style={{ padding: '30px 22px 14px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1 }}/>
        <Button theme={T} onClick={onNext} style={{ width: '100%' }}>{t('iAgree')}</Button>
      </div>
    </ScreenFrame>
  );
}
...
// ── Name + Avatar ─────────────────────────────────────────────
export function ScreenName({ theme, t, dir, state, onNext, onBack }) {
  const T = theme;
  const lang = dir === 'rtl' ? 'ar' : 'en';
  const [val, setVal] = useState(state.name || '');
  const [avatar, setAvatar] = useState(state.avatar || 'monogram');
    ...
    <ScreenFrame theme={T}>
      <div style={{ padding: '30px 22px 14px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ color: T.textMuted, fontSize: 14, lineHeight: 1.5, marginBottom: 26 }}>
          {lang === 'ar'
            ? 'اسمك يظهر فقط لك. فريقك يراك كـ "عضو مجهول" في لوحة المتصدرين.'
            : 'Your name is only visible to you. On leaderboards, your team sees "Anonymous member".'}
        </div>
        ...
        <div style={{ fontSize: 11, letterSpacing: 0.8, textTransform: 'uppercase', color: T.textMuted, marginBottom: 10, fontWeight: 600 }}>
          {lang === 'ar' ? 'الصورة الرمزية' : 'Avatar'}
        </div>
        ...
        <div style={{ flex: 1 }}/>
        <Button theme={T} onClick={submit} disabled={!canContinue} iconR="arrow" style={{ width: '100%' }}>
          {t('continue')}
        </Button>
      </div>
    </ScreenFrame>
...
// ── Baseline ──────────────────────────────────────────────────
export function ScreenBaseline({ theme, t, onNext, onBack, dir }) {
  const T = theme;
  const questions = [
  ...
  const [vals, setVals] = useState({ stress: 5, sleep: 6.5, energy: 5, mood: 6 });
      ...
      <div style={{ padding: '30px 22px 14px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <Card theme={T} pad={22} radius={22}>
          ...
          <div style={{ fontFamily: typeStyles(T).displayFont, fontSize: 56, lineHeight: 1, color: T.text, marginBottom: 26, letterSpacing: -1 }}>
            {q.fmt ? q.fmt(v) : v}
            <span style={{ color: T.textFaint, fontSize: 22 }}>{q.fmt ? '' : ` / ${q.max || 10}`}</span>
          </div>
        ...
        <div style={{ flex: 1 }}/>
        <Button theme={T} onClick={() => idx < questions.length - 1 ? setIdx(idx + 1) : onNext()} iconR="arrow" style={{ width: '100%' }}>
...
// ── Goals ─────────────────────────────────────────────────────
export function ScreenGoals({ theme, t, onNext, onBack, dir }) {
  const T = theme;
  const lang = dir === 'rtl' ? 'ar' : 'en';
  const goals = [
  ...
  const [sel, setSel] = useState(new Set(['stress', 'sleep']));
  const toggle = (id) => {
    const n = new Set(sel);
  ...
  return (
    <ScreenFrame theme={T}>
      <div style={{ padding: '30px 22px 14px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1 }}/>
        <Button theme={T} onClick={onNext} disabled={sel.size === 0} style={{ width: '100%' }}>{t('finish')}</Button>
      </div>
    </ScreenFrame>
  );
}
...
// ── Welcome ───────────────────────────────────────────────────
export function ScreenWelcome({ theme, t, state, onNext, dir }) {
  const T = theme;
  const lang = dir === 'rtl' ? 'ar' : 'en';
    ...
    <ScreenFrame theme={T}>
      <div style={{ padding: '30px 22px 14px', flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
        ...
        <div style={{
          ...
          {name ? (lang === 'ar' ? `أهلاً، ${name}` : `Welcome, ${name}`) : t('welcome')}
        </div>
