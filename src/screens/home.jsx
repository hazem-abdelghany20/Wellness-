import { typeStyles, Icon, AvatarDisplay, WellnessMark, Card, SectionLabel, Ring, Sparkline } from '../design-system.jsx';

export function IconBtn({ theme, icon, onClick }) {
  return (
...
export function ScreenHome({ theme, t, dir, go, variant = 'list', state }) {
  const T = theme;
          ...
          <Card theme={T} pad={14} style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: T.text, lineHeight: 1.2 }}>
                {lang === 'ar' ? 'خطة اليوم' : 'Today'}
              </div>
              <div style={{ fontSize: 11, color: T.textMuted, marginTop: 2 }}>
                {lang === 'ar' ? 'مهام مكتملة' : 'actions complete'}
              </div>
            </div>
          </Card>
        </div>
      ...
      {actions.map((a, i) => (
        <div key={a.id} onClick={() => a.id === 'breathe' ? go('breathe') : state.toggleAction(a.id)}
          ...
          <div style={{
            ...
            background: a.done ? T.accent : T.accentSoft,
            color: a.done ? T.accentInk : T.accent,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
        ...
        <Card key={a.id} theme={T} pad={18} radius={20}
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              ...
              <div style={{ fontSize: 10, letterSpacing: 1, textTransform: 'uppercase', color: T.textMuted, fontWeight: 600 }}>
