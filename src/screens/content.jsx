...
import { typeStyles, Icon, Card, Chip, SectionLabel } from '../design-system.jsx';
import { IconBtn } from './home.jsx';
import { TopBack } from './onboarding.jsx';

export function ScreenLibrary({ theme, t, dir, go }) {
  const T = theme;
  ...
  const items = [
    { id: 'sleep-onset', kind: 'audio',   mins: 6, cat: 'sleep',  title: { en: 'Sleep onset — a cue for tonight', ar: 'بداية النوم — إشارة لهذه الليلة' } },
      ...
      {cat === 'all' && (
        <div style={{ padding: '0 16px 18px' }}>
          <Card theme={T} pad={0} radius={24} style={{ overflow: 'hidden', cursor: 'pointer' }}
            ...
            <div style={{ height: 180, position: 'relative', background: `linear-gradient(135deg, ${T.accent}, ${T.accentSoft})` }}>
...
export function ScreenPlayer({ theme, t, dir, go, state }) {
  const T = theme;
  const lang = dir === 'rtl' ? 'ar' : 'en';
  ...
  return (
    <div style={{ height: '100%', background: T.bg, paddingTop: 54, boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '14px 22px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <TopBack theme={T} onBack={() => go('library')} dir={dir}/>
        <IconBtn theme={T} icon="plus" onClick={() => {}}/>
      </div>
...
export function ScreenNotifs({ theme, t, dir, go }) {
  const T = theme;
  const lang = dir === 'rtl' ? 'ar' : 'en';
  const sections = [
    {
