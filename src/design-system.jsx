...
// Two themes: 'brand' (deep green + amber) and 'light' (warm off-white)
...
export function Icon({ name, size = 22, stroke, fill = 'none', style = {} }) {
...
// ── Avatar ────────────────────────────────────────────────────
export const AVATAR_OPTIONS = ['monogram', 'orbit', 'wave', 'bloom', 'stone', 'ember'];

export function AvatarDisplay({ theme, kind = 'monogram', name = '?', size = 64 }) {
  const t = theme;
...
export function Button({ children, onClick, variant = 'primary', size = 'lg', theme, style = {}, disabled = false, icon, iconR }) {
  const t = theme;
  const sizes = {
...
export function Card({ theme, children, style = {}, onClick, pad = 16, radius = 20, alt = false }) {
  const t = theme;
  return (
...
export function Chip({ theme, children, active = false, onClick, icon, style = {} }) {
  const t = theme;
  return (
...
export function SectionLabel({ theme, children, right, style = {} }) {
  const t = theme;
  return (
    <div style={{
...
export function WellnessMark({ theme, size = 22, showText = true }) {
  const t = theme;
  const h = size;
  const w = h * (551 / 274);
  return (
...
export function Sparkline({ theme, values, height = 40, stroke, width = '100%' }) {
  const t = theme;
...
export function Ring({ theme, value = 0, size = 64, stroke = 6, color, bg, children }) {
  const t = theme;
...
export function Slider({ theme, value, onChange, min = 0, max = 10, step = 1, labels, accent }) {
  const t = theme;
